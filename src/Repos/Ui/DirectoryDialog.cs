...
public static class DirectoryDialog
{
    public static string? Open(string? title, string? inputPath, IntPtr owner = 0)
    {
        // ReSharper disable once SuspiciousTypeConversion.Global
        if (new FileOpenDialog() is not IFileOpenDialog dialog)
            return null;

        if (!string.IsNullOrEmpty(inputPath))
        {
            if (WinInterop.SHCreateItemFromParsingName(inputPath, null, typeof(IShellItem).GUID, out var item) != 0)
                return null;
        ...
        if (owner == IntPtr.Zero)
        {
            ...
            if (owner == IntPtr.Zero)
                owner = WinInterop.GetDesktopWindow();
        }

        var hr = dialog.Show(owner);
        if (hr == WinInterop.ERROR_CANCELLED)
            return null;

        if (hr != 0) return null;
        if (dialog.GetResults(out var items) != 0) return null;

        items.GetCount(out var count);
        if (count == 0)
            return null;

        items.GetItemAt(0, out var result);
        result.GetDisplayName(SIGDN.SIGDN_DESKTOPABSOLUTEPARSING, out var path);
        return path;
    }
